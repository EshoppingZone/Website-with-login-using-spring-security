	...
	<div class="col-sm-12">
		<nav class='navbar navbar-expand navbar-light bg-light'>
			...
			<div class="col-sm-4">
				...
				<c:if test="${pageContext.request.userPrincipal.name == null}">
					<li><a href="http://localhost:8080/homelogin" style="margin-left: 32px" class="">Sign
							In</a></li>
							</c:if>
		<form id="logoutForm" method="POST" action="${contextPath}/logout">
            <input type="hidden" name="${_csrf.parameterName}" value="${_csrf.token}"/>
							...
							<c:if test="${pageContext.request.userPrincipal.name != null}">
        <li style="margin-left: 32px" class="">Welcome ${pageContext.request.userPrincipal.name}</li>
        
        <li><button><a onclick="document.forms['logoutForm'].submit()">Logout</a></button></li>
